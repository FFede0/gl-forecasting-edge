...
def load_all_logs(gossip_dir: str) -> pd.DataFrame:
  logs = pd.DataFrame()
...
if __name__ == "__main__":
  ...
  for event, event_logs in logs.groupby("event"):
    ...
    # average and sum averages over epochs
    print(80 * "-")
    print(f"Event: {event}")
    print(avg.sum())
    pd.DataFrame(avg.describe()).to_csv(
      os.path.join(gossip_dir, f"{event}_metrics_summary.csv")
    )
